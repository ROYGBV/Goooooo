/*
Задание:
- настроить postgresql на порт 5432
- создание таблицы: url, method, responseCode
- подключение sqlx и драйвера
- подключиться к таблице из кода в отдельном main, сделать insert данных, затем select из таблицы и вывод данных
- *не инициировать подключение на каждый запрос, а держать постоянный коннект и взаимодействовать через него
- *сделать в докере
*/
...
func makeMultipleRequestsHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Работа с БД
	DBaction(reqAm, decodedRequestList, resArr)
}
...
func makeSingleRequestHandler(w http.ResponseWriter, r *http.Request) {
	...
	reqAm := 1
	var decodedRequestList RequestList
	decodedRequestList.Requests = append(decodedRequestList.Requests, decodedData)
	DBaction(reqAm, decodedRequestList, resArr)
}
